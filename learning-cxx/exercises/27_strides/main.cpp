...
std::vector<udim> strides(std::vector<udim> const &shape) {
    std::vector<udim> strides(shape.size());  //now strides is {0,0,0}
    // TODO: 完成函数体，根据张量形状计算张量连续存储时的步长。
    // READ: 逆向迭代器 std::vector::rbegin <https://zh.cppreference.com/w/cpp/container/vector/rbegin>
    //       使用逆向迭代器可能可以简化代码
    
    std::vector<udim>::iterator ite = strides.begin(); 
    std::vector<udim>::iterator ite2 = strides.begin(); 
    int cnt = 0; 
    for(auto& ele: shape){   //iterator can't be applied to 'const reference type' shape       
        if(cnt > 0){
            *ite = ele;
            while(ite2 < ite){
                *ite2 *= ele;

...
bool key_exists(std::map<k, v> const &map, k const &key) {
    // TODO: 实现函数
    auto ite = map.find(key);  //typename std::map<k,v>::iterator itr = map.find(key), there is error.  auto is a good choice.
    return ite != map.end();
}
...
void set(std::map<k, v> &map, k key, v value) {
    // TODO: 实现函数
    //map.insert(std::pair<k, v>(key, value));  for an existing key, insert will not overide the ori value.
    map[key] = value;
}

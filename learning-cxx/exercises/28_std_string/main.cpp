...
int main(int argc, char **argv) {
    ...
    auto hello = "Hello"s;
    auto world = "world";
    //std::cout << decltype(hello).name() << std::endl;     error
    //std::cout << typeid(hello).name() << std::endl;       PKc 
    ...
    decltype(world) w2;
    ASSERT((std::is_same_v<decltype(hello), decltype("Hello"s)>), "Fill in the missing type.");

    ASSERT((std::is_same_v<decltype(world), decltype(w2)>), "Fill in the missing type.");
    // TODO: 将 `?` 替换为正确的字符串
